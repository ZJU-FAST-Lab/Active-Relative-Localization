...
/*------------------------gyzt的代码start--------------------------*/
geometry_msgs::PoseStamped offsetPoseROS;
nav_msgs::Path             offsetpathROS;
ros::Publisher offsetpathPub;
bool   offset_origin       = false;
bool   isOffsetOriginSet  = false;
colvec offsetPoseOrigin(6);
/*------------------------gyzt的代码end--------------------------*/
...
/*------------------------gyzt的代码start--------------------------*/
void odom_offset_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  if (msg->header.frame_id == string("null"))
    return;
  ...
  if (offset_origin && !isOffsetOriginSet)
  {
    isOffsetOriginSet = true;
    offsetPoseOrigin  = pose;
  }
  if (origin)
  {
    vel  = trans(ypr_to_R(pose.rows(3,5))) * vel;  
    pose = pose_update(pose_inverse(offsetPoseOrigin), pose);
    vel  = ypr_to_R(pose.rows(3,5)) * vel;  
  }

  // Pose
  offsetPoseROS.header = msg->header;
  offsetPoseROS.header.stamp = msg->header.stamp;
  offsetPoseROS.header.frame_id = string("world");
  offsetPoseROS.pose.position.x = pose(0);
  offsetPoseROS.pose.position.y = pose(1);
  offsetPoseROS.pose.position.z = pose(2);
  q = R_to_quaternion(ypr_to_R(pose.rows(3,5)));
  offsetPoseROS.pose.orientation.w = q(0);
  offsetPoseROS.pose.orientation.x = q(1);
  offsetPoseROS.pose.orientation.y = q(2);
  offsetPoseROS.pose.orientation.z = q(3);     
  ...
  {
    prevt = msg->header.stamp;
    offsetpathROS.header = offsetPoseROS.header;
    offsetpathROS.poses.push_back(offsetPoseROS);
    offsetpathPub.publish(offsetpathROS);
  }
}
/*------------------------gyzt的代码end--------------------------*/
...
int main(int argc, char** argv)
{
  ...
  /*------------------------gyzt的代码start--------------------------*/
  ros::Subscriber sub_odom_offset = n.subscribe("odom_offset", 100, odom_offset_callback);
  offsetpathPub = n.advertise<nav_msgs::Path>("offsetpath", 100, true);
  /*------------------------gyzt的代码end--------------------------*/
