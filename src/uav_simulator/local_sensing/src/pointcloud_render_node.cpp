...
ros::Subscriber noise_odom_sub; // new code 
...
nav_msgs::Odometry _odom; // true odom
nav_msgs::Odometry _noise_odom; // noise odom
...
void noiseOdometryCallbck(const nav_msgs::Odometry& odom) {
  _noise_odom = odom;
}
...
void renderSensedPoints(const ros::TimerEvent& event) {
  ...
  if (_kdtreeLocalMap.radiusSearch(searchPoint, sensing_horizon,
    ...
    for (size_t i = 0; i < _pointIdxRadiusSearch.size(); ++i) {
      ...
      /*-------------------------------new code start---------------------------------*/
      pt.x += _noise_odom.pose.pose.position.x - _odom.pose.pose.position.x; // point cloud under noise_odom
      pt.y += _noise_odom.pose.pose.position.y - _odom.pose.pose.position.y; 
      pt.z += _noise_odom.pose.pose.position.z - _odom.pose.pose.position.z; 
      /*-------------------------------new code end---------------------------------*/
...
int main(int argc, char** argv) {
  ...
  noise_odom_sub = nh.subscribe("noise_odometry", 50, noiseOdometryCallbck);
