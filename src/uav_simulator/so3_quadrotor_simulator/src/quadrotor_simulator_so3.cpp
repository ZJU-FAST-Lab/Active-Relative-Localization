...
bool master_no_noise_ = false; 
const int window_size = 1500;
std::deque<double> x_ran_window;
std::deque<double> y_ran_window;
std::deque<double> z_ran_window;
...
nav_msgs::Odometry first_odom_msg;
...
main(int argc, char** argv)
{
  ...
  /*-------------------------------new code start---------------------------------*/
  ros::Publisher  real_pos_pub = n.advertise<nav_msgs::Odometry>("real_pos", 100);
  /*-------------------------------new code end---------------------------------*/
  ...
  /*-------------------------------new code start---------------------------------*/
  n.param("simulator/x_noise_step", x_step, 2.0);
  n.param("simulator/y_noise_step", y_step, 2.0);
  n.param("simulator/z_noise_step", z_step, 0.5);
  n.param("simulator/master_no_noise", master_no_noise_, false);
  /*-------------------------------new code end---------------------------------*/
  ...
  */
  ...
  while (n.ok())
  {
    ...
    static std::default_random_engine e(rd());
	  static std::normal_distribution<double> x_d(0, x_step);
	  static std::normal_distribution<double> y_d(0, y_step);
	  static std::normal_distribution<double> z_d(0, z_step);
    /*-------------------------------new code end---------------------------------*/
    ...
    if (tnow >= next_odom_pub_time)
    {
      ...
      /*-------------------------------new code start---------------------------------*/
      nav_msgs::Odometry fake_odom_msg = odom_msg;
      if(first_odom) {
        first_odom_msg = odom_msg;
        last_odom_msg = odom_msg;
        first_odom = false;
      }

      double delta_dist = sqrt(pow(fabs(odom_msg.pose.pose.position.x - last_odom_msg.pose.pose.position.x), 2) +
                        pow(fabs(odom_msg.pose.pose.position.y - last_odom_msg.pose.pose.position.y), 2) +
                        pow(fabs(odom_msg.pose.pose.position.z - last_odom_msg.pose.pose.position.z), 2));
      // double dist = sqrt(pow(fabs(odom_msg.pose.pose.position.x - first_odom_msg.pose.pose.position.x), 2) +
      //                   pow(fabs(odom_msg.pose.pose.position.y - first_odom_msg.pose.pose.position.y), 2) +
      //                   pow(fabs(odom_msg.pose.pose.position.z - first_odom_msg.pose.pose.position.z), 2));
      // x_ran += x_d(e) * sqrt (2 * dist * delta_dist);
      // y_ran += y_d(e) * sqrt (2 * dist * delta_dist);
      // z_ran += z_d(e) * sqrt (2 * dist * delta_dist);
      x_ran += x_d(e) * delta_dist;
      y_ran += y_d(e) * delta_dist;
      z_ran += z_d(e) * delta_dist;

      x_ran_window.push_back(x_ran);
      y_ran_window.push_back(y_ran);
      z_ran_window.push_back(z_ran);

      if (x_ran_window.size() > window_size)
        x_ran_window.pop_front();
      if (y_ran_window.size() > window_size)
        y_ran_window.pop_front();
      if (z_ran_window.size() > window_size)
        z_ran_window.pop_front();

      double x_ran_smoothed = std::accumulate(x_ran_window.begin(), x_ran_window.end(), 0.0) / x_ran_window.size();
      double y_ran_smoothed = std::accumulate(y_ran_window.begin(), y_ran_window.end(), 0.0) / y_ran_window.size();
      double z_ran_smoothed = std::accumulate(z_ran_window.begin(), z_ran_window.end(), 0.0) / z_ran_window.size();

      fake_odom_msg.pose.pose.position.x = odom_msg.pose.pose.position.x + x_ran_smoothed;
      fake_odom_msg.pose.pose.position.y = odom_msg.pose.pose.position.y + y_ran_smoothed;
      fake_odom_msg.pose.pose.position.z = odom_msg.pose.pose.position.z + z_ran_smoothed;

      std::string frame_id = odom_msg.child_frame_id;
      int id = frame_id[frame_id.size()-1] - '0';

      if (id == 0 && master_no_noise_) {
        odom_pub.publish(odom_msg); 
      } else {
        odom_pub.publish(fake_odom_msg); 
      }

      real_pos_pub.publish(odom_msg); 
      last_odom_msg = odom_msg;
      /*-------------------------------new code end---------------------------------*/
